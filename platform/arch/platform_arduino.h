 ...
 *      Authors: murilotrevisan e Vitor Alexandre(@XandGVaz)
 *
 *  Description:
 ...
 */
 ...
 typedef struct {
   ...
   // Pino onde será modulado o sinal analógico
   uint8_t target;
 } pwm_handle_t;  
 ...
 EXPORT error_t pwm_start(pwm_t pwm){
   return(!ledcAttachChannel(pwm.handle.target, pwm.handle.frequency, 
         pwm.bits, pwm.channel)); // Anexação do pino ao canal PWM, com dadas frequência e precisão 
 }
